...
const dataElems = filteredData.splice(0,10).map(elems => {
    return (
      ...
      element={<Crypto/>}
      >
        <li className='flex flex-row justify-between py-8 px-4 text-center border-b border-double hover:bg-indigo-600 hover:bg-opacity-10 items-center xs-max:justify-center' key={elems.id} >
            ...
            <p className='text-2xl items-center flex sm-max:text-xl xs-max:hidden'>$ {elems.current_price.toLocaleString('en-US')}</p>
    ...
    <section id='market' className="market bg-gradient-to-b from-custom-start to-custom-start">
        <div className="market__container flex flex-col py-20 px-20 xs-max:px-10">
            <div className="market__content">
                ...
                <input 
                  type="text" 
                  className='mb-10 w-full bg-indigo-600 bg-opacity-10 p-3 text-xl xs-max:text-sm border-solid border-2 border-white rounded-xl focus:outline-none focus:border-purple-600' 
                  value={search}
                ...
                <div className="market__list flex flex-col">
                    ...
                    <div className="list__top flex flex-row justify-between bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 p-4 rounded-t-lg whitespace-break-spaces xs-max:justify-center">
                        <p className='text-2xl'>Coin</p>
                        <p className='text-2xl xs-max:hidden'>Price</p>
