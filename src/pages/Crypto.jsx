...
const Crypto = () => {
  ...
  return (
    ...
    <section className='crypto w-full h-screen bg-gradient-to-b from-chart-end to-chart-start overflow-hidden xl-max:overflow-auto'>
        ...
        <div className="crypto__content flex justify-between items-center p-20 pt-24 lg-max:grid lg-max:p-10 lg-max:grid-cols-1"
         onLoad={() => setLoad(false)}>
          
      
          {load? (<Loader/>) : 
          (<div className="content__card flex flex-col items-center text-left gap-3 border-2 p-6 rounded-3xl bg-indigo-600 bg-opacity-5 border-indigo-950 ">

              {coin.image ? <img src={coin.image.large} className='w-24 h-24 mb-3' alt=""/> : null }
              <h2 className='text-3xl '>{coin.name}</h2>
          
              <ul className="chart__statistics text-left mt-3 text-md">
